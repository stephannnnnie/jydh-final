...
def train(args):
    ...
    train_log_path = '%s_vgg_no.txt' % (args.data_name)
    ...
    tr_dataset = mydata(img_path=args.train_data_path, img_size=args.img_size, km_file_path=args.km_file_path,
    ...
    if args.test_with_train:
        te_dataset = mydata(img_path=args.test_data_path, img_size=args.img_size, km_file_path=args.km_file_path,
                            color_info=args.color_info)
    ...
    discriminator = Discriminator(args.input_channel + args.output_channel, args.color_feat_dim, args.img_size).to(
        device)
    ...
    for e in range(args.epoch):
        ...
        f.write(' %04d-epoch is best train model' % (best_model))
        f.write(' g_loss : %04f \t d_loss : %04f \n' % (g_loss.item(), d_loss.item()))

        if args.test_with_train and (e + 1) % args.test_freq == 0:
            generator.eval()
            L = test_operation(args, generator,mem, te_dataloader, device, e)
            f.write(' LPIPS : %04f\n' % (L))
            # test_operation(args, generator, te_dataloader, device, e)
            generator.train()
        f.close()
...
def test(args):
    ...
    test_dataset = mydata(img_path=args.test_data_path, img_size=args.img_size, km_file_path=args.km_file_path,
                          color_info=args.color_info)
    ...
    L = test_operation(args, generator,mem, test_dataloader, device)
...
def test_operation(args, generator,mem, te_dataloader, device, e=-1):
    ...
    with torch.no_grad():
        for i, batch in enumerate(te_dataloader):
            ...
            gray_image = torch.cat([l_channel * 100, torch.zeros((bs, 2, args.img_size, args.img_size)).to(device)],
                                   dim=1).cpu().numpy()

            loss_fn = lpips.LPIPS(net='alex')
            a = np.transpose(real_image, (0, 2, 3, 1))
            a = np.asarray([lab2rgb(ele) for ele in a])
            a = (a * 255.0).astype(np.uint8)
            b = np.transpose(fake_image, (0, 2, 3, 1))
            b = np.asarray([lab2rgb(ele) for ele in b])
            b = (b * 255.0).astype(np.uint8)
            transf = transforms.ToTensor()
            for i in range(a.shape[0]):
                d = loss_fn(transf(a[i]), transf(b[i]))
                print(d)
                if L is not None:
                    L = torch.cat((L, d), dim=-1)
                else:
                    L = d
