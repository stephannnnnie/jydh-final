...
def train(args):
    ...
    for e in range(args.epoch):
        ...
        if args.test_with_train and (e + 1) % args.test_freq  == 0:
            generator.eval()
            L=test_operation(args, generator,  te_dataloader, device, e)
            f.write(' LPIPS : %04f\n' % (L))
...
def test(args):
    ...
    L=test_operation(args, generator,test_dataloader, device)




def test_operation(args, generator, te_dataloader, device, e = -1):
    count = 0
